...
namespace reflect
{
	...
	MetaClass* MetaInfo::RegisterMetaClass(MetaClass& meta)
	{
		const auto& it = MetaRegistry.find(meta.FilePath);
		if (it->second.HasMetaClass(meta.Name))return nullptr;
		const std::string name = meta.Name;
		it->second.Classes.emplace_back(std::move(meta));
		return &CurFile->GetMetaClass(name);
	}
